...
describe ResourceRecord do
  describe '.unpack' do
    ...
    let(:ip_address) { '203.171.34.211' }
    let(:packed_ip_address) { ip_address.split('.').map(&:to_i).pack('cccc') }
    ...
    context 'A record' do
      ...
      it 'unpacks correctly' do
        ...
        expect(resource_record.to_s).to eq(ip_address)
        expect(packed_stream.tell).to eq(49)
      end
    end

    context 'NS record' do
      let(:ns1_address) { 'ns-1523.awsdns-62.org' }

      before do
        packed_stream.seek(49)
      end

      it 'unpacks correctly' do
        expect(resource_record.name.to_s).to eq('powershop.co.nz')
        expect(resource_record.to_s).to eq(ns1_address)
        expect(packed_stream.tell).to eq(84)
      end
    end
  end
end
